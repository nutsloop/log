...
log::stream_redirect_::stream_redirect_(std::ostream &stream_out,
                                        std::ostream &stream_err)
    : original_cout_(stream_out.rdbuf()), original_cerr_(stream_err.rdbuf()) {

  this->cout_file_.open(nutsloop::nutsloop_logs_directory / "nutsloop_cout.log",
                        std::ios::out | std::ios::app);
  this->cerr_file_.open(nutsloop::nutsloop_logs_directory / "nutsloop_cerr.log",
                        std::ios::out | std::ios::app);
  ...
  if (!this->cout_file_.is_open()) {
    ...
    { // MARK (LOG) MUTEX LOCK
      ...
      internal_debug_->stream(__FILE__, __LINE__, ERROR)
          << "log::stream_redirect_::stream_redirect_() failed to open "
             "cout<<log file '"
          << nutsloop::nutsloop_logs_directory / "nutsloop_cout.log" << "'"
          << std::endl;
    }
#endif
  ...
  if (!this->cerr_file_.is_open()) {
    ...
    { // MARK (LOG) MUTEX LOCK
      ...
      internal_debug_->stream(__FILE__, __LINE__, ERROR)
          << "log::stream_redirect_::stream_redirect_() failed to open "
             "cerr<<log file '"
          << nutsloop::nutsloop_logs_directory / "nutsloop_cerr.log" << "'"
          << std::endl;
    }
#endif
